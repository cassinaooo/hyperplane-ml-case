...
# %%
def extract_ngrams(s):
    return (
        set(["".join(bi) for bi in nltk.bigrams(s)]),
        set(["".join(tri) for tri in nltk.trigrams(s)]),
    )


# %%
print(extract_ngrams(np.random.choice(norm_vendors_str)))

# %%
vendors_ngrams = {}

for vendor in norm_vendors_str:
    bigrams, trigrams = extract_ngrams(vendor)
...
for s in np.random.choice(norm_vendors_str, samples):
    print(s, "->", vendors_ngrams[s]["ngrams"])
...
for i in range(samples):
    ...
    print(
        ...
        b,
        "->",
        set_similarity(vendors_ngrams[a]["ngrams"], vendors_ngrams[b]["ngrams"]),
    )
...
def compute_similarities(vendor_1, cuttoff=0.5):
    ...
    for vendor_2 in norm_vendors_str:
        if vendor_1 != vendor_2:
            sim = set_similarity(
                vendors_ngrams[vendor_1]["ngrams"],
                vendors_ngrams[vendor_2]["ngrams"],
            )
...
# %%
with open("./data/vendors_similarities.pkl", "wb") as f:
